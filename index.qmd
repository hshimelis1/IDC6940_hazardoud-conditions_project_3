...
Model performance was evaluated using 5-fold cross-validation using
the boot package [@boot1] [@boot2].

#### Model selection

The Concordance (c-index), AIC, and BIC of corresponding models were
compared to select the best fit model. The model with the lowest AIC and BIC values and highest concordance
...
time to surgery columns were replaced with the descriptive names. Evaluation of nodes and node4 showed that samples with greater than 4 lymph nodes have less than 5 count in nodes column, so the two columns are not consistent. Therefore,
...
The coefficient of Lev is not significant, suggesting that
...
longer survival time. The hazard ratio for Lex+5FU (0.690), indicating the risk of death is about 31% lower compared to the observation group. 
...
```
The Schoenfeld residal plot shows that the residuals are
