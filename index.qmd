...
The R MASS package was used for Stepwise variable selection, using
"both" forward and backward variable selection [@MASS]. For Stepwise
variable selection, stepAIC() function uses AIC (Akaike Information
...
The Concordance (c-index), AIC, and Baysian Information Criterion (BIC)
...
A stepwise variable selection approach was used to confirm that the
...
Based on a stepwise selection, rx, age, surg, obstruct, differentiation,
node4 and local spread were selected for inclusion in the final model.
Age is a significant predictor in the stepwise-selected variables, but
not in the full variable model (Model_2).
